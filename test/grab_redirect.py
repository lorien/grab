...
class GrabRedirectTestCase(BaseGrabTestCase):
    ...
    def test_meta_refresh_redirect(self):
        ...
        self.server.response_once['get.data'] = '<meta http-equiv="refresh" content="5; url=%s">' % meta_url
        ...
        self.server.response_once['get.data'] = "<meta http-equiv='refresh' content='0;url= %s'>" % meta_url
        g = Grab(transport=GRAB_TRANSPORT)
        g.setup(follow_refresh=True)
        g.go(self.server.get_url())
        self.assertEqual(self.server.request['path'], '/foo')
