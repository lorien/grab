...
class DeprecatedThings(object):
    ...
    def rex(self, regexp, flags=0, byte=False, default=UNSET):
    ...
    def find_link(self, href_pattern, make_absolute=True):
        ...
        if isinstance(href_pattern, six.text_type):
            raise GrabMisuseError(
                "Method `find_link` accepts only " "byte-string argument"
            )
    ...
    def xpath(self, path, default=UNSET):
        try:
            return self.doc.select(path).node(
                default=UNDEFINED if default is UNSET else default
            )
        except SelectionNotFoundError:
            raise DataNotFound

    @deprecated(use_instead="grab.doc.select().one()")
    def xpath_one(self, path, default=UNSET):
        try:
            return self.doc.select(path).node(
                default=UNDEFINED if default is UNSET else default
            )
    ...
    def xpath_text(self, path, default=UNSET, smart=False, normalize_space=True):
        try:
            return self.doc.select(path).text(
                default=UNDEFINED if default is UNSET else default,
                smart=smart,
    ...
    def xpath_number(
        self, path, default=UNSET, ignore_spaces=False, smart=False, make_int=True
    ):
        try:
            return self.doc.select(path).number(
                default=UNDEFINED if default is UNSET else default,
                smart=smart,
    ...
    @deprecated()
    def css_one(self, path, default=UNSET):
        """
    ...
    @deprecated()
    def css_text(self, path, default=UNSET, smart=False, normalize_space=True):
        """
        ...
        try:
            return get_node_text(
                self.css_one(path), smart=smart, normalize_space=normalize_space
            )
        except DataNotFound:
            if default is UNSET:
                raise
    ...
    def load_cookies(self, path, file_required=None):
        if file_required is not None:
            warn("The option `file_required` is no longer supported", stacklevel=3)
    ...
    @deprecated(use_instead="grab.proxylist.load_file OR " "grab.proxylist.load_url")
    def load_proxylist(
        self, source, source_type, proxy_type="http", auto_init=True, auto_change=True
    ):
    ...
    def form(self):
        warn(
            "The `Grab.form` attribute is deprecated. " "Use `Grab.doc.form` instead.",
            stacklevel=3,
        )
