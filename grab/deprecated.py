...
class DeprecatedThings(object):
    ...
    def xpath_text(self, path, default=NULL, smart=False, normalize_space=True):
        return self.doc.select(path).text(
            default=default, smart=smart, normalize_space=normalize_space
        )

    @deprecated(use_instead="grab.doc.select().number()")
    def xpath_number(
        self, path, default=NULL, ignore_spaces=False, smart=False, make_int=True
    ):

        return self.doc.select(path).number(
            default=default, smart=smart, ignore_spaces=ignore_spaces, make_int=make_int
        )
    ...
    def css_one(self, path, default=NULL):
        ...
        try:
            ...
            if default is NULL:
                raise DataNotFound("CSS path not found: %s" % path) from ex
            else:
                return default
