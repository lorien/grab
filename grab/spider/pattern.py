...
class SpiderPattern(object):
    ...
    def process_object_image(self, task_name, collection, obj, image_field,
                             image_url, base_dir, ext='jpg',
                             skip_existing=True):
        ...
        if os.path.exists(path) and skip_existing:
            ...
            self.add_task(Task(task_name, url=image_url, obj=obj,
                               disable_cache=True, image_field=image_field,
                               collection=collection,
                               base_dir=base_dir, ext=ext))

    def generic_task_image(self, grab, task):
        relpath = grab.response.save_hash(task.url,
                                          task.base_dir, ext=task.ext)
        path = os.path.join(task.base_dir, relpath)
        task.collection.update(
            {'_id': task.obj['_id']},
