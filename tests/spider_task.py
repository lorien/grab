...
class TestSpiderTestCase(BaseGrabTestCase):
    ...
    #    self.assertEqual(req.headers.get("User-Agent"), "Foo")
    ...
    #    self.server.add_response(Response(status=502), count=2)
    ...
    #            grab.setup(timeout=const_timeout)

    #        def task_generator(self):
    #            yield Task("page", url=self.meta["server"].get_url())
    #            yield Task(
    #                "page", grab=Grab(url=self.meta["server"].get_url(), timeout=0.1)
    #            )
    ...
    #    bot.setup_queue()
    #    bot.add_task(Task("page", url=self.server.get_url()))
    #    bot.add_task(Task("page", grab=Grab(url=self.server.get_url(), timeout=0.1)))
    #    bot.run()
    #    self.assertEqual(set([const_timeout]), set(bot.stat.collections["points"]))
    ...
    def test_worker_restored(self):
        ...
        bot = build_spider(TestSpider, parser_requests_per_process=2)
        bot.setup_queue()
        for _ in range(5):
            bot.add_task(Task("page", url=self.server.get_url()))
        bot.run()
    ...
    def test_response_not_valid(self):
        ...
        bot.setup_queue()
        bot.add_task(Task("page", url=self.server.get_url()))
        self.server.add_response(Response(), count=bot.task_try_limit)
        bot.run()
