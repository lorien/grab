...
class TestCookies(BaseGrabTestCase):
    ...
    def test_log_option(self):
        with temp_dir() as tmp_dir:
            ...
            self.server.add_response(Response(data="abc"), count=1, method="get")

            self.assertEqual(os.listdir(tmp_dir), [])
            grab.go(self.server.get_url())
            self.assertEqual(os.listdir(tmp_dir), ["lograb.html"])
            with open(log_file_path) as inp:
                self.assertEqual(inp.read(), "abc")
    ...
    def test_log_dir_option(self):
        with temp_dir() as tmp_dir:
            ...
            self.server.add_response(Response(data="abc1"), count=1, method="get")
            self.server.add_response(Response(data="abc2"), count=1, method="get")
            ...
            with open(os.path.join(tmp_dir, "01.html")) as inp:
                self.assertEqual(inp.read(), "abc1")
            with open(os.path.join(tmp_dir, "02.html")) as inp:
                self.assertEqual(inp.read(), "abc2")
    ...
    def test_log_dir_response_content(self):
        with temp_dir() as tmp_dir:
            ...
            self.server.add_response(
                Response(data="abc", headers=[("X-Engine", "PHP")]), count=1
            )
    ...
    def test_debug_post(self):
        ...
        self.server.add_response(Response(data="x"), count=1, method="post")
        grab.go(self.server.get_url())
    ...
    def test_debug_post_integer_bug(self):
        ...
        self.server.add_response(Response(data="x"), method="post")
        grab.go(self.server.get_url())
        self.assertEqual(b"x", grab.doc.body)
    ...
    #    grab.go(self.server.get_url())
    #    req = self.server.get_request()
    #    self.assertEqual(req.data, ("foo=%s" % big_value).encode())
